...
- `lineMapping`: customer → allowed production lines (from `customer_line_mapping` table)
...
## Headcount & Overtime Recommendations
...
Default to `skip: false` for all orders unless there is a clear business reason to exclude an order.
